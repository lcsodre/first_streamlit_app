...
def get_databases_load_list():
  ...
  with my_cnx.cursor() as my_cur:
    my_cur.execute("select DATABASE_NAME FROM SNOWFLAKE.INFORMATION_SCHEMA.DATABASES order by DATABASE_NAME")
...
def get_tables_load_list(p_catalog):
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
  with my_cnx.cursor() as my_cur:
    v_query="SELECT TABLE_SCHEMA || '.' || TABLE_NAME AS NAME FROM " +p_catalog+ ".INFORMATION_SCHEMA.TABLES order by TABLE_SCHEMA,TABLE_NAME"
...
def get_domains_load_list():
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
  with my_cnx.cursor() as my_cur:
    v_query="SELECT DOMAIN_ID || '|' || DOMAIN_NAME FROM DMDQFMRWK.METADATA.DOMAINS order by DOMAIN_NAME"
    my_cur.execute(v_query)
    f_return=my_cur.fetchall() 
    my_cnx.close()
    df = pd.DataFrame(f_return,columns=['Name'])
...
def get_responsibles_load_list():
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
  with my_cnx.cursor() as my_cur:
    v_query="SELECT RESPONSIBLE_ID || '|' || RESPONSIBLE_NAME FROM DMDQFMRWK.METADATA.RESPONSIBLES order by RESPONSIBLE_NAME"
    my_cur.execute(v_query)
    f_return=my_cur.fetchall() 
    my_cnx.close()
    df = pd.DataFrame(f_return,columns=['Name'])
...
def get_structures_load_list():
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
  with my_cnx.cursor() as my_cur:
    v_query="SELECT STRUCTURE_ID || '|' || STRUCTURE_NAME FROM DMDQFMRWK.METADATA.STRUCTURES order by STRUCTURE_NAME "
    my_cur.execute(v_query)
    f_return=my_cur.fetchall() 
    my_cnx.close()
    df = pd.DataFrame(f_return,columns=['Name'])
